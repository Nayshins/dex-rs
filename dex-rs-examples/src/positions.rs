...
async fn main() -> DexResult<()> {
    ...
    for (i, position) in positions.iter().enumerate() {
        ...
        println!("   Unrealized PnL: {} ${:.2}", pnl_emoji, position.unrealized_pnl);
        ...
        if let Some(entry_px) = position.entry_px {
            if entry_px > 0.0 && position.size != 0.0 {
                ...
                let roe_emoji = if roe > 0.0 { "🟢" } else if roe < 0.0 { "🔴" } else { "⚪" };
    ...
    sorted_positions.sort_by(|a, b| {
        b.unrealized_pnl.abs().partial_cmp(&a.unrealized_pnl.abs()).unwrap_or(std::cmp::Ordering::Equal)
    });
    
    if sorted_positions.len() > 1 {
        println!("\n🎯 Top Performers by PnL:");
        for (i, position) in sorted_positions.iter().take(3).enumerate() {
            let pnl_emoji = if position.unrealized_pnl > 0.0 { "🟢" } else { "🔴" };
            println!("   {}. {} {} ${:.2}", 
                     i + 1, 
                     position.coin, 
                     pnl_emoji, 
