...
async fn main() -> DexResult<()> {
    ...
    let margin_used: f64 = user_state
        ...
        .parse()
        .unwrap_or(0.0);
    let ntl_pos: f64 = user_state
        .cross_margin_summary
        .total_ntl_pos
        .parse()
        .unwrap_or(0.0);
    let raw_usd: f64 = user_state
        .cross_margin_summary
        .total_raw_usd
        .parse()
        .unwrap_or(0.0);
    let maintenance_margin: f64 = user_state
    ...
    if !user_state.asset_positions.is_empty() {
        println!(
            "\n📊 Asset Positions ({} total):",
            user_state.asset_positions.len()
        );
        println!("{:-<80}", "");
        println!(
            "{:<8} {:<12} {:<15} {:<15} {:<15}",
            "Asset", "Size", "Entry Price", "Position Value", "PnL"
        );
        println!("{:-<80}", "");
    ...
    if health_ratio < 1.2 {
        println!(
            "   🔴 Account Health: {:.2} (DANGER - close to liquidation)",
            health_ratio
        );
    } else if health_ratio < 2.0 {
        println!(
            "   🟡 Account Health: {:.2} (WARNING - monitor closely)",
            health_ratio
        );
    } else {
