...
async fn main() -> DexResult<()> {
    ...
    let coin = "BTC";
    println!("💹 Subscribing to live BBO (Best Bid/Offer) for {}...", coin);
    ...
    println!("{:-<90}", "");
    println!("{:<12} {:<15} {:<15} {:<15} {:<10} {:<15}", "Time", "Bid", "Ask", "Mid", "Spread", "Change");
    println!("{:-<90}", "");
    
    loop {
        match rx.recv().await {
            Some(StreamEvent::Bbo { coin: _coin, bid_px, ask_px, timestamp }) => {
                update_count += 1;
...
fn show_statistics(price_history: &VecDeque<f64>, update_count: usize) {
    ...
    let max_price = price_history.iter().fold(f64::NEG_INFINITY, |a, &b| a.max(b));
