        ...
        match timeout(Duration::from_secs(60), rx.recv()).await {
            Ok(Some(event)) => {
                if let StreamEvent::L2(orderbook) = event {
                    ...
                    println!(
                        "📖 Order Book: {} | Update #{} | {}",
                        orderbook.coin, update_count, timestamp
                    );
                    ...
                    let best_bid = orderbook
                        .bids
                        .first()
                        .map(|b| b.price.into_inner())
                        .unwrap_or(0.0);
                    let best_ask = orderbook
                        .asks
                        .first()
                        .map(|a| a.price.into_inner())
                        .unwrap_or(0.0);
                    ...
                    let total_bid_vol: f64 = orderbook
                        .bids
                        .iter()
                        .take(depth_display)
                        .map(|b| b.qty.into_inner())
                        .sum();
                    let total_ask_vol: f64 = orderbook
                        .asks
                        .iter()
                        .take(depth_display)
