...
async fn main() -> DexResult<()> {
    ...
    let mut asset_stats: std::collections::HashMap<String, (f64, f64, usize)> =
    ...
    for fill in fills.iter().take(50) {
        // Show last 50 fills
        ...
        println!(
            ...
            datetime, side_emoji, fill.coin, price, size, fee, fill.tid
        );
    ...
    if fills.len() > 50 {
        println!(
            "... and {} more fills (showing most recent 50)",
            fills.len() - 50
        );
    }
    ...
    if !fills.is_empty() {
        ...
        if recent_liquidations > 0 {
            println!(
                "⚠️ Warning: {} recent liquidation(s) detected",
                recent_liquidations
            );
        }
    }
