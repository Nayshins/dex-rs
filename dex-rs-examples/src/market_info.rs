...
async fn main() -> DexResult<()> {
    ...
    println!("{:-<80}", "");
    println!("{:<10} {:<15} {:<12} {:<15} {:<15}", "Symbol", "Mid Price", "Max Lev", "Oracle Price", "Mark Price");
    println!("{:-<80}", "");
    
    for (i, asset) in meta.assets.iter().enumerate().take(20) {  // Show first 20 assets
        let mid_price = all_mids.mids.get(&asset.name)
    ...
    let mut volume_assets: Vec<_> = meta_and_contexts.asset_ctxs.iter()
        .enumerate()
        .filter_map(|(i, ctx)| {
    ...
    for asset in &meta.assets {
        if asset.only_isolated {
            println!("  🔒 {} (Isolated only, max {}x leverage)", asset.name, asset.max_leverage);
        }
    }
